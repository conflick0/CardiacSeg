...
from networks.EfficientSegNet.blocks.context_block import AnisotropicAvgPooling
from networks.EfficientSegNet.blocks.residual_block import ResBaseConvBlock


class EfficientSegNet_X1(nn.Module):
  def __init__(self, in_channels, out_channels, feature_size=8, inp_size=[96,96,96]):
    super().__init__()
    ...
    # context block (neck block)
    context_block = AnisotropicAvgPooling
    context_kernel_size = [i // 16 for i in inp_size]
